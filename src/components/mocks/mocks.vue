  ...
  <div style="overflow: hidden" class="exam-detail">
    <div>
      ...
      <div class="content">
        ...
        <div class="exam-info" v-else>
          <div>{{ examInfo.name }}</div>
          <!-- <div>{{ currentIdx + 1 + "/" + examInfo.total }}</div> -->
          <!-- <div>{{ '共 ' + examInfo.total + ' 道题' }}</div> -->
        </div>
        <div v-if="list.length > 0" class="question-list">
          <div v-for="(l, index) in list" :key="l.questionId">
            <div class="question" v-if="l.questionTypeId == 1">
              <van-cell
                :value="index + 1 + '、' + l.title"
                style="font-weight: bold"
              ></van-cell>
              <van-radio-group v-model="l.myAnswer">
                <van-cell-group>
                  <van-cell
                    :value="item.label + '、' + item.content"
                    clickable
                    @click="l.myAnswer = item.label"
                    v-for="item in l.choiceList"
                    :key="item.label"
                  >
                    <template #right-icon>
...
export default {
  ...
  methods: {
    ...
    async getExamInfo(id) {
      ...
      this.loading = false;
      if (result.respCode != 200) {
        Toast.fail(result.respMessage || '加载试题失败，请重试!');
      } else {
